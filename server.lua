...
QBCore.Functions.CreateCallback('dw-garages:server:CheckOwnership', function(source, cb, plate)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then 
        return cb(false) 
    end

    -- Direct check using MojiaVehicleKeys server function
    local hasKey = exports['MojiaVehicleKeys']:HasKey(plate)  -- هادي طريقة صحيحة فـ server
    cb(hasKey or false)
end)
...
QBCore.Functions.CreateCallback('dw-garages:server:GetVehicleLocation', function(source, cb, plate)
    local Player = QBCore.Functions.GetPlayer(source)
    MySQL.query('SELECT * FROM player_vehicles WHERE plate = ?', { plate }, function(result)
        if result[1] then
            cb({ x = result[1].x, y = result[1].y, z = result[1].z })
        else
            cb(nil)
        end
    end)
...
QBCore.Functions.CreateCallback('dw-garages:server:GetPlayerVehicles', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    local Vehicles = {}
        ...
        if result and #result > 0 then
            for _, v in ipairs(result) do
                ...
                if VehicleData then
                    ...
                    print("undefined vehicle: " .. tostring(v.vehicle))
                end
            end
        end
        cb(Vehicles)
...
function ParkJobVehicle(vehicle, jobName)
    ...
    CreateThread(function()
      ...
      TriggerClientEvent('dw-garages:client:ParkJobVehicle', src, vehicleNetId, parkingSpot, engineHealth, bodyHealth, fuelLevel, plate)
...
local hasKey = exports['MojiaVehicleKeys']:HasKey(plate)

if not hasKey then
    TriggerClientEvent('QBCore:Notify', src, 'ما عندكش المفتاح ديال هاد السيارة!', 'error')
    return
end

-- إذا عندو المفتاح، نحيدو المفتاح من MojiaVehicleKeys
...
RegisterNetEvent('dw-garages:server:UpdateVehicleState', function(plate, state, garageCoords)
    MySQL.Async.execute('UPDATE player_vehicles SET state = ?, last_update = ? WHERE plate = ?', 
        {state, os.time(), plate}, 
        function(rowsChanged)
            if rowsChanged > 0 then
                if state == 0 then
                    OutsideVehicles[plate] = garageCoords
                else
                    OutsideVehicles[plate] = nil
                end
            else
...
QBCore.Functions.CreateCallback('dw-garages:server:GetVehicleLocation', function(source, cb, plate)
    ...
    if vehicleData then
        cb({ success = true, coords = vehicleData })
    else
        cb({ success = false, coords = nil })
    end
end)
...
-- source و plate معرفين قبل
TriggerEvent('MojiaVehicleKeys:server:HasKey', plate, function(hasKey)
    if not hasKey then
        TriggerClientEvent('QBCore:Notify', src, 'ما عندكش المفتاح ديال هاد السيارة!', 'error')
        return
    end

    -- إزالة المفتاح من MojiaVehicleKeys
    TriggerEvent('MojiaVehicleKeys:server:RemoveKey', plate)

    -- دابا باقي الكود ديال تخزين السيارة في shared garage
    local citizenid = Player.PlayerData.citizenid
