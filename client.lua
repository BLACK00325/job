...
-- تعريف دالة round باش ما يبقاش الخطأ
function round(num, numDecimalPlaces)
    local mult = 10^(numDecimalPlaces or 0)
    return math.floor(num * mult + 0.5) / mult
end

-- مثال FormatVehiclesForNUI
function FormatVehiclesForNUI(vehicles)
    ...
    for i, vehicle in ipairs(vehicles) do
        ...
        if vehicleInfo then
            ...
            local displayName = vehicle.custom_name ~= "" and vehicle.custom_name or vehicleInfo.name
...
function OpenImpoundUI(vehicles, impoundInfo, impoundId)
    ...
    for i, vehicle in ipairs(vehicles) do
        ...
        if vehicleInfo then
            ...
            local displayName = vehicle.custom_name ~= "" and vehicle.custom_name or vehicleInfo.name
            local totalFee = tonumber(vehicle.impoundfee) or Config.ImpoundFee
            local reasonString = vehicle.impoundreason or "No reason specified"
            if #reasonString > 50 then reasonString = reasonString:sub(1,47).."..." end
...
-- التعامل مع مفتاح السيارة عبر Callback بدلاً من Export
function HasVehicleKey(plate, cb)
...
RegisterNUICallback('takeOutVehicle', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('dw-garages:server:GetVehicleByPlate', function(vehData, isOut)
        ...
        QBCore.Functions.SpawnVehicle(model, function(veh)
            ...
            local veh = NetToVeh(vehNet) -- السيارة لي تسپاونات
local plate = QBCore.Functions.GetPlate(veh)

-- زيد هاد السطر باش تعطي الساروت للمستخدم
TriggerServerEvent("MojiaVehicleKeys:server:givekey", plate)
...
local function HasKey(plate)
    ...
    return exports['MojiaVehicleKeys']:HasKey(plate)
end

-- عند محاولة إخراج السيارة من الجراج:
if not HasKey(vehiclePlate) then
    QBCore.Functions.Notify("ما عندكش المفتاح ديال هاد السيارة!", "error")
    return
end
